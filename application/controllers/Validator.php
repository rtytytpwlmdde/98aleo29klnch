...
class Validator extends CI_Controller{
	...
	function peminjaman_non_rutin(){
		...
		$data['pegawaiTelp']=$this->Mvalidator->get_telp_pegawai();
		$data['mahasiswaTelp']=$this->Mvalidator->get_telp_mahasiswa();
		$data['peminjaman_non_rutin'] = $this->Mvalidator->get_all_data_peminjaman_non_rutin_wakadek();
		...
		$data['pegawaiTelp']=$this->Mvalidator->get_telp_pegawai();
		$data['mahasiswaTelp']=$this->Mvalidator->get_telp_mahasiswa();
		$data['peminjaman_non_rutin'] = $this->Mvalidator->get_all_data_peminjaman_non_rutin($akses);
...
//alvin
	...
	function hapus_ruangan_peminjaman_khusus($id_peminjaman,$id_ruangan){
		...
		$this->m_admin->hapus_data($where,'detail_peminjaman_non_rutin');
		$this->session->set_flashdata('notif', "Ruangan berhasil dihapus");
		redirect('validator/input_ruangan_peminjaman_khusus/'.$id_peminjaman);
	}
	//
function get_option_penyelenggara(){
	...
	function ubah_status_peminjaman_non_rutin($jenis,$id_peminjaman,$id_peminjam,$penyelenggara){
		if($jenis == 1){
			$status = "setuju";
			if($penyelenggara == '2'){
				$status_wadek = "setuju";
			}else{
				$status_wadek = "terkirim";
			}

			$data_non_rutin = array(
				'status' => $status,
				'status_wakadek' => $status_wadek
			);
			$data_peminjaman = array(
				'status_peminjaman' => $status,
				'status_peminjaman_wakadek' => $status_wadek
			);
			$where_non_rutin = array('id_peminjaman_non_rutin' => $id_peminjaman);
